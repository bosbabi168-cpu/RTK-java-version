...
case "${1:-}" in
    ...
    all)
        ...
        start_one map
        ;;
    scripttest|maptest|chartest|worldtest|cliftest|dbtest|luaaudit|wiresync|elixirtest|carnagetest|sumotest|beachtest)
        need_jar
        mode="$1"; shift
    ...
    status)
        ...
        ;;
    *)
        echo "Usage: $0 {login|char|map|all|scripttest|maptest|chartest|worldtest|cliftest|elixirtest|carnagetest|sumotest|beachtest|dbtest|luaaudit|status|stop}"
        exit 1
        ;;
esac
