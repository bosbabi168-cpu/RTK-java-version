metal_bags = {
	...
	presentOptions = function(player)
		...
		local choices = {}
		if bag.dura ~= bag.maxDura then
			table.insert(choices, "Masukkan logam ke tas.")
		end
		if bag.dura ~= 0 then
			table.insert(choices, "Keluarkan logam dari tas.")
		end

		local choice = player:menuString("Apa yang ingin kau lakukan?", choices)

		if choice == "Masukkan logam ke tas." then
			metal_bags.addToBag(player, amberType)
		elseif choice == "Keluarkan logam dari tas." then
	...
	use = async(function(player)
		...
		if #foundMetal == 0 then
			player:dialogSeq(
				{
					t,
					"Kau harus punya metal di kantongmu untuk bisa menambahkannya ke tas."
				},
				0
			)
			return
		...
		if invItem ~= nil then
			...
			if invItem.amount > 1 then
				amount = player:inputNumberCheck(player:input("Berapa banyak " .. invItem.name .. " yang ingin kau masukkan ke kantong logam?"))
